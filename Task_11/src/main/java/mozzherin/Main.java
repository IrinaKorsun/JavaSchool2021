...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        bank.createAccounts(numberOfAccounts, totalAmount, args.length == 2 && args[1].equals("BAD_TEST"));

        Thread threadDeamon = new Thread(new Deamon(bank, totalAmount));
        threadDeamon.setDaemon(true);
        threadDeamon.start();

        ExecutorService executor = Executors.newFixedThreadPool(numberOfClients);
        for (int i = 0; i < numberOfClients; i++) {
            executor.execute(new Client(bank, (int) (Math.random() * totalAmount / numberOfAccounts),
